...
2. Figure out exactly what you are doing before you type. State your assumptions: "add authentication" is several different tasks, so name them explicitly. Name the tradeoffs when they matter to the approach. If something is truly vague, stop and ask rather than filling the gap with plausible-looking code; that is exactly the code that passes a casual review and fails when it matters.
