...
6. Add a doctest example to pure-ish functions (data in, data out).
...
2. Prefer **declarative design** over imperative approaches. From a single variable to an entire system, if you can express it declaratively upfront, do so. Make the full picture visible straight up and avoid requiring readers to dive into implementation. Embedding flow and logic in sprawling implementations creates difficulty.
...
1. **Every task needs a provable success criterion before code is written**. "Add validation" becomes "reject a missing or malformed email, return 400 with a clear message, and test both cases." Break larger tasks down by **which checkpoints have to be _PROVEN_** before you can commit to an approach.
2. Figure out exactly what you are doing before you type. State your assumptions: "add authentication" is several different tasks, so name them explicitly. Name the tradeoffs when they matter to the approach. If something is unequivocally unclear, stop and ask rather than filling the gap with plausible-looking code; that is exactly the code that passes a casual review and fails when it matters.
...
On your end, write ASD-STE100 Simplified English-flavored prose. Ignore the ‘technical’ in the name, and apply the ‘simplified’ principles on all the prose/responses you write. 
WORDS
...
When you write the summary at the end, drop the working shorthand. This is the best opportunity to apply ASD-STE100-flavored easy-to-read prose. 
