...
Codex-internal `notes` and `history` calls carry only ciphertext. They become out-of-context `codex-encrypted-call` custom entries, in sequence. A `collaboration` call is ciphertext only in its `message` field, so it stays a normal tool call: task names, agent types, models, and `wait_agent` results reach the Pi model, and every ciphertext string in the arguments becomes a placeholder. Verify results with `scripts/pi-goldload.mjs`. A ported compaction makes `reached` smaller than `expected` by design.
